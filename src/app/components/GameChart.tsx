...
import { Card, CardBody, Box, Heading, Tabs, TabList, Tab, TabPanels, TabPanel } from '@chakra-ui/react'
...
export default function GameChart({ rounds, playerCount }: GameChartProps) {
  ...
  const lakeData = rounds.map((round, index) => ({
    round: round.numero,
  ...
  const sustainabilityData = rounds.map((round, index) => ({
    round: round.numero,
              ...
              <Box h="300px">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={lakeData}>
                    ...
                    />
                    <YAxis
                      label={{ value: 'Fish Count', angle: -90, position: 'insideLeft' }}
                      stroke="#4A5568"
                    />
                    <Tooltip
                      contentStyle={{
